...
data_2017 <- finaldata[finaldata$Year == 2017, ]

# Count the number of missing values in the Maternal_Mortality column
missing_mat_mor <- sum(is.na(data_2017$Maternal_Mortality))

# Display the result
missing_mat_mor

# Filter the dataset for 2017 and exclude rows with missing Maternal_Mortality
data_2017 <- finaldata[finaldata$Year == 2017 & !is.na(finaldata$Maternal_Mortality), ]

# Count countries exposed to armed conflict (assuming 'armconf1' indicates exposure)
exposed_to_conflict <- sum(data_2017$armconf1 == 1, na.rm = TRUE)

# Count countries not exposed to armed conflict
not_exposed_to_conflict <- sum(data_2017$armconf1 == 0, na.rm = TRUE)

# Display results
cat("In 2017, there were", exposed_to_conflict, "countries exposed to armed conflict, and", 
    not_exposed_to_conflict, "countries not exposed to armed conflict, excluding those with missing maternal mortality data.\n")

# Filter the dataset for 2017, exclude rows with missing Maternal_Mortality, and select countries exposed to armed conflict
conflict_data_2017 <- finaldata[finaldata$Year == 2017 & 
                                !is.na(finaldata$Maternal_Mortality) & 
                                finaldata$armconf1 == 1, ]

# Calculate the median of Maternal_Mortality
median_maternal_mortality <- median(conflict_data_2017$Maternal_Mortality, na.rm = TRUE)

# Display the result
cat("The sample median of the 2017 maternal mortality rate (per 100,000 live births) in countries exposed to armed conflict is:", data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABIAAAASCAYAAABWzo5XAAAAbElEQVR4Xs2RQQrAMAgEfZgf7W9LAguybljJpR3wEse5JOL3ZObDb4x1loDhHbBOFU6i2Ddnw2KNiXcdAXygJlwE8OFVBHDgKrLgSInN4WMe9iXiqIVsTMjH7z/GhNTEibOxQswcYIWYOR/zAjBJfiXh3jZ6AAAAAElFTkSuQmCC
    median_maternal_mortality, "\n")
